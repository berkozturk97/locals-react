...
export const basketReducer = (state = initialState, { type, payload }) => {
  switch (type) {
    ...
    case types.REMOVE_ITEM_FROM_BASKET: {
      const selectedItem = state.basketItems.find((basketItem) => basketItem.item.name === payload.name);

      if (selectedItem.quantity === 1) {
        const filteredBasket = state.basketItems.filter((basketItem) => basketItem.item.name !== payload.name);
